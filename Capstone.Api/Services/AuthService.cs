...
public sealed class AuthService
{
    ...
    public async Task<(bool ok, object? data, string error)> ManagementLoginAsync(LoginRequest req)
    {
        if (string.IsNullOrWhiteSpace(req.Email) || string.IsNullOrWhiteSpace(req.Password))
            return (false, null, "Please enter your email and password.");
...
", new { UserId = userId });
        ...
        if (string.IsNullOrWhiteSpace(role))
            throw new Exception("Your account is not set up yet. Please contact support.");
...
", new { RoleName = role }))
    ...
    public async Task<(bool ok, object? data, string error)> LoginByUserIdAsync(int userId)
    {
        await using var conn = _db.Create();
...
", new { UserId = userId });
        ...
        if (user is null)
            return (false, null, "Your session has expired. Please sign in again.");
    ...
    public async Task<(bool ok, string error)> ChangePasswordAsync(int userId, ChangePasswordRequest req)
    {
        if (string.IsNullOrWhiteSpace(req.CurrentPassword) || string.IsNullOrWhiteSpace(req.NewPassword))
            return (false, "Please enter your current password and a new password.");
