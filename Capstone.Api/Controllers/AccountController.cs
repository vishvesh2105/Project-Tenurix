...
", new { UserId = userId });
    ...
    public async Task<IActionResult> UpdateMe([FromBody] UpdateMyProfileRequest req)
    {
        if (string.IsNullOrWhiteSpace(req.FullName))
            return BadRequest(new ApiError("Please enter your full name."));
...
", new
    ...
    public async Task<IActionResult> UploadMyPhoto()
    {
        ...
        if (!Request.HasFormContentType)
            return BadRequest(new ApiError("Please upload a photo file."));

        var form = await Request.ReadFormAsync();
        var file = form.Files.FirstOrDefault();

        if (file == null || file.Length == 0)
            return BadRequest(new ApiError("Please select a photo to upload."));
